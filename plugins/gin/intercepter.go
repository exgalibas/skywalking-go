...
func (h *ContextInterceptor) BeforeInvoke(invocation operator.Invocation) error {
	...
	tracing.SetRuntimeContextValue("can_root", struct{}{})
	defer tracing.SetRuntimeContextValue("can_root", nil)
	s, err := tracing.CreateEntrySpan(
		...
		tracing.WithTag(tracing.TagURL, context.Request.URL.String()),
